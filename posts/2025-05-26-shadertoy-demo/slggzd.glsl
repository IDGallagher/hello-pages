// Buffer A implementation of Shadertoy slGGzD
...
vec4 R(float x, float y) {
  return texture(textureState, vec2(x, y));
}
...
void main() {
  ...
  vec4 s = R(x,y);
  if (length(s) == 0.0 || (iMouse.z>0.0 && length(iMouse.xy/Scale-fragCoord)<20.0)) {
    gl_FragColor = hash43(vec3(fragCoord, iTime)) - 0.5;
    return;
  }
  ...
  gl_FragColor = clamp(s + ds, -1.5, 1.5);
}
